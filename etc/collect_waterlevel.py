...
def main():
    ...
    for Obs_NM in Obs_NMs:
        ...
        # Read SE_Hydrograph
        hydro_file = f"{base_dir}/{Obs_NM}_{bestTrail:02d}/best_Raven/output/SE_WaterLevels.csv"
        try:
    ...
    mk_dir(output_dir)
    df.to_csv(f"{output_dir}/SE_WaterLevels.csv", index=False)
    print(f"Saved merged Water Levels to {output_dir}/SE_WaterLevels.csv")
